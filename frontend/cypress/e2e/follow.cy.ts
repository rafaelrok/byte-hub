...
describe('follow', () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', /profiles\/\S+/, { fixture: 'profile.json' }).as(
      'getProfile',
    )
    cy.fixture('article.json').then((article) => {
      article.article.author.username = 'foo'
      cy.intercept('GET', /articles\/\S+/, {
        statusCode: 200,
        body: article,
      }).as('getArticle')
    })
  ...
  it('should not display follow button when user not logged', () => {
    ...
    cy.get('.article-meta button.space:first').should(
      'not.contain.text',
      'Follow',
    )
  })
  ...
  it('should call follow user api when click follow user button', () => {
    cy.fixture('profile.json').then((profile) => {
      ...
      cy.intercept('POST', /profiles\/\S+\/follow/, {
        statusCode: 200,
        body: profile,
      }).as('followUser')
    })
    ...
    cy.wait('@followUser')
    cy.get('.article-actions button.btn-outline-secondary:last').should(
      'contain',
      'Unfollow',
    )
  })
