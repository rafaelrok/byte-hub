...
describe('article', () => {
  beforeEach(() => {
    ...
    cy.intercept('DELETE', /articles\/.+/, { statusCode: 200, body: {} }).as(
      'deleteArticle',
    )
  })
  ...
  describe('post article', () => {
    it('jump to post detail page when submit create article form', () => {
      ...
      cy.url().should('contain', 'article/article-title')
    ...
    it('should render markdown correctly', () => {
      ...
      cy.get('.article-content').within(() => {
        ...
        cy.get('strong').should('contain', 'Strong')
      })
    })
  })
