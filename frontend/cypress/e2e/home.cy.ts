...
describe('homepage', () => {
  beforeEach(() => {
    cy.intercept('GET', /articles\?tag=butt/, {
      fixture: 'articles-of-tag.json',
    }).as('getArticlesOfTag')
    cy.intercept('GET', /articles\?limit/, { fixture: 'articles.json' }).as(
      'getArticles',
    )
    cy.intercept('GET', /articles\/.+/, { fixture: 'article.json' }).as(
      'getArticle',
    )
  ...
  it('should display article when page loaded', () => {
    cy.visit(ROUTES.HOME)
    cy.get('.article-preview:first').find('h1').should('contain.text', 'abc123')
  })

  it('should read more information of the first articles', () => {
    cy.visit(ROUTES.HOME)
    cy.get('div.article-preview:first span').contains('Read more...').click()

    cy.get('.article-page').contains(' to add comments on this article. ')
  })
  ...
  it('should jump to next page when click page 2 in pagination', () => {
    ...
    cy.get('ul.pagination li.page-item:nth-child(2) a.page-link').click()
