...
describe('# EditArticle page', () => {
  ...
  it('should call create api when fill form and click submit button', async () => {
    ...
    await fireEvent.update(
      getByPlaceholderText('Article Title'),
      'Article Title',
    )
    await fireEvent.update(
      getByPlaceholderText("What's this article about?"),
      'Article descriptions',
    )
    await fireEvent.update(
      getByPlaceholderText('Write your article (in markdown)'),
      'this is **article body**.',
    )
    await userEvent.type(
      getByPlaceholderText('Enter tags'),
  ...
  it('should call update api when click submit button and in editing', async () => {
    ...
    const mockedRequest = await server.waitForRequest(
      'PUT',
      '/api/articles/article-foo',
    )

    expect(router.push).toHaveBeenCalledWith({
      name: 'article',
      params: { slug: 'article-foo' },
    })
    expect(await mockedRequest.json()).toMatchInlineSnapshot(`
      {
        "article": {
