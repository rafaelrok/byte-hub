...
describe('# ArticleDetail', () => {
  const server = setupMockServer(
    [
      ...
      { article: { ...fixtures.article, body: fixtures.markdown } },
    ],
    [
      '/api/articles/markdown-cn',
      { article: { ...fixtures.article, body: fixtures.markdownCN } },
    ],
    [
      '/api/articles/markdown-xss',
      { article: { ...fixtures.article, body: fixtures.markdownXss } },
    ],
  )

  it('should render markdown body correctly', async () => {
    const { container } = render(
      asyncWrapper(ArticleDetail),
      await renderOptions({
