...
import type { Profile } from 'src/services/api.ts'
...
describe('# ArticleDetailMeta', () => {
  ...
  it('should display follow button when user not author', () => {
    const { getByRole, queryByRole } = render(
      ...
      renderOptions({
        initialState: {
          user: { user: { ...fixtures.user, username: 'user2' } },
        },
        props: { article: fixtures.article },
      }),
    )
  ...
  it('should call favorite article service when click favorite button', async () => {
    server.use([
      ...
      { article: { ...fixtures.article, favorited: true } },
    ])
    const { getByRole } = render(
      ArticleDetailMeta,
      renderOptions({
  ...
  it('should call favorite article service when click unfavorite button', async () => {
    server.use([
      'DELETE',
      '/api/articles/*/favorite',
      { article: { ...fixtures.article, favorited: false } },
    ])
    const { getByRole } = render(
      ArticleDetailMeta,
      renderOptions({
