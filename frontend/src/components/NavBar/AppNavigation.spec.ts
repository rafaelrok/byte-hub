...
describe('# AppNavigation', () => {
  ...
  it('should render xxx when user logged', () => {
    ...
    expect(getByRole('link', { name: 'Home' })).toHaveTextContent('Home')
    expect(getByRole('link', { name: 'New Post' })).toHaveTextContent(
      'New Post',
    )
    expect(getByRole('link', { name: 'Settings' })).toHaveTextContent(
      'Settings',
    )
    expect(getByRole('link', { name: 'username' })).toHaveTextContent(
      'username',
    )
  })
})
